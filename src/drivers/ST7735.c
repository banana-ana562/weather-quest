...
void LCD_OutChar(char ch, uint8_t size){
	uint8_t maxCols = (_width  / (6 * size)) - 1;  // e.g. 128/(6*2)-1 = 9
  uint8_t maxRows = (_height / (10 * size)) - 1; // e.g. 160/(10*2)-1 = 7
  ...
  LCD_DrawCharS(StX*6*size,StY*10*size,ch,ST7735_YELLOW,ST7735_BLACK, size);
  StX++;
  if(StX>maxCols){
    StX = maxCols;
    LCD_DrawCharS(StX*6*size,StY*10*size,'*',ST7735_RED,ST7735_BLACK, size);
  }
  return;
}

void LCD_OutString(char *ptr, uint8_t size){
  while(*ptr){
    LCD_OutChar(*ptr, size);
    ptr = ptr + 1;
  }
}
